...
| **P3** | Phase 5 — Web Clipper / External Ingest (PWA share_target, URL import with SSRF hardening, PDF chunking, Chrome MV3 extension with limited-scope token) | session `plan.md` |
| **P3** | Phase 6 — Knowledge Graph + Bidirectional Linking (note_links triple-uniqueness, title/aliases, backlinks, manual links, wiki-link `[[Title]]` parsing + backfill) | session `plan.md` |
